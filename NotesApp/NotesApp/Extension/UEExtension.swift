...
extension UE {
    struct Data: Identifiable{
        ...
        public var moyenne: CGFloat
        public var nbUE: Int
        public var listeNotes: [MatiereVM]
    }
    
    var data: Data{(Data(id: id, name: name, coef: coef, moyenne: moyenne.floatToCgFloat(), nbUE: nbUE, listeNotes: listeNotes.map({MatiereVM(matiere: $0)})))}
    ...
    mutating func update(data: Data){
        ...
        self.coef = data.coef
        self.nbUE = data.nbUE
        self.listeNotes = data.listeNotes.map({Matiere(id: $0.original.id, name: $0.original.name, coef: $0.original.coef, note: $0.original.note)})
    }
}*/
